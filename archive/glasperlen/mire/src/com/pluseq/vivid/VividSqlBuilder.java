...
public class VividSqlBuilder {
	/**
	 * Converts String[] to 'Peter','John','Marry'
	 */
	public static String escape(String[] array){
	 ...
	 */
	public static String escapeOnly(String s){
	    int length = s.length();
	...
	{
		VividSqlParts p = partsFromValues(values);
		StringBuffer strBuf = new StringBuffer(prefix + " ");
		strBuf.append(tableName);
		strBuf.append("(");
		
		for (VividField field : p.whatFields) {
	...
	public VividSqlParts partsFromValues(Hashtable<String,Object> values)
	{
		VividSqlParts p = new VividSqlParts();
		for (String key : values.keySet()) {
			p.whatFields.add(new VividField(key));
